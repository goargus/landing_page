<template>
  <div class="relative mt-4 overflow-hidden h-[680px]">
    <div
      class="flex transition" :style="`transform: translateX(-${currentIndex * 100}%)`">
      <div
        ...
        <img :src="project.imagesrc" :alt="project.imageAlt"
          class="image-size"/>
        <a :href="project.link"
          class="transition button-website"
          target="_blank">Ver Website</a>
      </div>
    </div>
    <button
      @click="prevSlide" class="transform arrow-left">
      <img src="../assets/arrow.svg" alt="Arrow left" class="w-139 h-189" />
    </button>
    <button
     @click="nextSlide"class="transform arrow-right">
